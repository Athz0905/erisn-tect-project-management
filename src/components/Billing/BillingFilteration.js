...
const BillingFilteration = ({ history }) => {
  ...
  useEffect(() => {
    axios
      .get(
        "https://freshhu.com/cnu/projectmanagement/api/admin/projectManagement/installments?paidStatus=0",
        {
          headers: {
  ...
  const currentPosts = BillingFilteration.slice(offset, offset + PER_PAGE);
